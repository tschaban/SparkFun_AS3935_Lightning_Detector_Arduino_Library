/*
  ...
  Modifications to meet AFE Firmware requrments by https://github.com/tschaban
  - initializing sensor with an address and wire at one shot
  - SPI removed, only I2C is needed (to decrease the size of the lib)
...
bool SparkFun_AS3935::begin(i2cAddress address, TwoWire &wirePort) {
...
/* NOT required for AFE Firmware
bool SparkFun_AS3935::beginSPI(uint8_t user_CSPin, uint32_t spiPortSpeed,
                               SPIClass &spiPort) {
  // Startup time requires 2ms for the LCO and 2ms more for the RC oscillators
  // which occurs only after the LCO settles. See "Timing" under "Electrical
  // Characteristics" in the datasheet.
  ...
  _i2cPort = NULL;
  _spiPort = &spiPort;
  _spiPortSpeed = spiPortSpeed; // Make sure it's not 500kHz or it will cause
                                // feedback with antenna.
  _cs = user_CSPin;
  pinMode(_cs, OUTPUT);
...
// the the TRCO will need to be recalibrated: REG0x08[5] = 1, wait 2 ms,
// REG0x08[5] = 0.
...
// after power down. The following function wakes the IC, sends the "Direct
// Command" to
// CALIB_RCO register REG0x3D, waits 2ms and then checks that it has been
// successfully
// calibrated. Note that I-squared-C and SPI are active during power down.
bool SparkFun_AS3935::wakeUp() {

  _writeRegister(AFE_GAIN, POWER_MASK, 0,
                 0); // Set the power down bit to zero to wake it up
...
void SparkFun_AS3935::watchdogThreshold(uint8_t _sensitivity) {
  if ((_sensitivity < 1) ||
      (_sensitivity > 10)) // 10 is the max sensitivity setting
    return;
...
// Increasing this value increases robustness at the cost of sensitivity to
// distant events.
...
// When there is an event that exceeds the watchdog threshold, the register is
// written
// with the type of event. This consists of two messages: INT_D (disturber
// detected) and
...
// noise has ended. Events are active HIGH. There is a one second window of time
// to
...
bool SparkFun_AS3935::calibrateOsc() {
  ...
  _writeRegister(CALIB_RCO, WIPE_ALL, DIRECT_COMMAND,
                 0); // Send command to calibrate the oscillators
...
void SparkFun_AS3935::_writeRegister(uint8_t _wReg, uint8_t _mask,
                                     uint8_t _bits, uint8_t _startPosition) {
...
/* NOT required for AFE Firmware                                       
  if (_i2cPort == NULL) {
    ...
    _spiWrite |=
        (_bits << _startPosition); // Write the given bits to the variable
...
/* NOT required for AFE Firmware
  if (_i2cPort == NULL) {
    _spiPort->beginTransaction(SPISettings(_spiPortSpeed, MSBFIRST, SPI_MODE1));
    digitalWrite(_cs, LOW); // Start communication.
    _spiPort->transfer(_reg |=
                       SPI_READ_M);    // Register OR'ed with SPI read command.
    ...
    */
    ...
    _i2cPort->endTransmission(false); // 'False' here sends a restart message so
                                      // that bus is not released
